...
```
core/agent_runner.py        HelloAgents装配：ReActAgent + MCP工具 + 双模型路由
core/maa_mcp_server.py      MCP Server：截图/OCR/点击/滑动/BFS自动导航，游戏无关
core/exploration_memory.py  探索记忆：节点+操作图，跨会话持久化
core/safety_guard.py        坐标白名单 + 敏感词整屏拦截（硬护栏）
core/risk_gate.py           单个候选点击的风险评估 + 待确认队列（软风控）
core/react_agent_vision.py  给ReActAgent打补丁：多模态图片消息 + 长短期记忆压缩
core/usage_log.py           使用记录：按game_id记录每次跑了什么模式/输入/结果，
                             不同app互不干扰

core/progress_estimate.py   进度估计：frontier_progress + discovery_rate双指标
core/idle_scheduler.py      空闲自动探索，用户活动时让位
core/intent_router.py       意图→目标定位：已知技能/语义状态搜索/目标导向探测
core/skill_store.py         技能库：成功路径可以固化成可复用技能
core/execute_intent.py      意图执行入口，串起上面这几个模块

core/navigate_demo.py       真机BFS导航演示（不经过LLM）
core/review_pending.py      人工审阅risk_gate记录的待确认队列，选择性放行
core/dedupe_nodes.py        离线批量合并重复节点
core/cleanup_graph.py       离线整理悬空边/重复动作/孤立节点
core/visualize_graph.py     把探索图谱渲染成可拖拽/缩放的本地网页
```
...
`core/risk_gate.py`的敏感词/不可逆语义）会跳过、记入待确认队列，不阻塞探索——用
`core/review_pending.py`可以事后审阅、选择性放行执行。
...
```
./.venv/Scripts/python core/execute_intent.py games/无期迷途/profile.yaml "去商城看看"
```
...
```
./.venv/Scripts/python core/idle_scheduler.py games/无期迷途/profile.yaml [空闲阈值秒] [轮询间隔秒]
```

持续运行，空闲超过阈值且`core/progress_estimate.py`判断"还没探完"时自动拉起一段自由探索；
检测到用户活动（`agent_runner.py`带指令跑一次就会记一次）立刻终止当前探索，让位给用户。

## 查看使用记录

```
./.venv/Scripts/python core/usage_log.py <game_id>            # 汇总：总次数/按模式分组/最近一次
./.venv/Scripts/python core/usage_log.py <game_id> --all      # 加上完整历史
./.venv/Scripts/python core/usage_log.py <game_id> --context  # 压缩后可直接喂给LLM的版本
```
...
最近几条留完整细节，更早的压缩成按模式/结果分组的聚合统计，跟`core/react_agent_vision.py`
的长短期记忆压缩是同一个思路。

## 探索图谱：查看与整理

```
./.venv/Scripts/python core/visualize_graph.py <game_id>          # 渲染成可拖拽/缩放的网页
./.venv/Scripts/python core/dedupe_nodes.py <game_id>             # 离线合并重复节点
./.venv/Scripts/python core/cleanup_graph.py <game_id>            # 清理悬空边/重复动作
./.venv/Scripts/python core/cleanup_graph.py <game_id> --delete-isolated   # 连孤立节点一起清
./.venv/Scripts/python core/navigate_demo.py games/无期迷途/profile.yaml <目标node_id>  # 真机BFS导航演示
```
