...
也存在同一目录下（<node_id>.png，core/device/maa_mcp_server.py里screenshot()工具负责
...
class ExplorationMemory:
    ...
    def similarity_between(self, node_id_a: str, node_id_b: str) -> float:
        """两个已知节点之间的文字相似度，给core/tools/dedupe_nodes.py这类离线批量
    ...
    def visit(
        ...
        *,
        ...
        处理图像数据，调用方（比如core/device/maa_mcp_server.py，手头有当前截图和
    ...
    def path_to(self, from_node_id: str, to_node_id: str) -> list[str] | None:
        ...
        给core/device/navigate_demo.py这类"导航到某个具体已知节点"的场景用——跟
